...
def fetch_public_key(device_name, pka_private_key):
    """Fetch the public key of a device from the PKA and decrypt it."""
    # Send a POST request to fetch the public key
    response = requests.post(
        "http://localhost:6000/get_public_key",
        json={
            "device": device_name,
            "client_id": "A"
        }
    ...
    if response.status_code == 200:
        # Extract the encrypted public key (n, e, ciphertext) from the response
        ...
        print("Failed to get public key:", response.json().get("error"))
        return None

# Function to encrypt a message using a public key
def rsaencrypt(plaintext, public_key):
    n, e = public_key
    e = e.replace('\x00', '').strip()  # Remove null byte and strip any extra spaces
    n = n.strip()  # Remove any extra spaces
    e = int(e)
    n = int(n)
...
if __name__ == '__main__':
    client_program()
# print(fetch_public_key("B", public_key_pka))