...
# Device A public key (n=3233, e=17)
public_key_a = (3233, 17)
# Device B public key (n=4933, e=17)
public_key_b = (391, 17)


app = Flask(__name__)

# Pre-generate key pairs for devices
device_keys = {
    "A": public_key_a,
    "B": public_key_b
}
...
def get_public_key():
    ...
    if device in device_keys:
        # Retrieve the public key of the requested device
        public_key = device_keys[device]
        print(f"Device {device} public key: {public_key}")

        # Encrypt the device public key using PKA's public key
        # Convert the public key (n, e) to a string and encrypt it
        public_key_str = f"{public_key[0]},{public_key[1]}"  # Convert to string representation
        encrypted_public_key = encrypt(public_key_str, public_key_pka)
        print(f"Encrypted public key: {encrypted_public_key}")

        return jsonify({"encrypted_public_key": encrypted_public_key})
